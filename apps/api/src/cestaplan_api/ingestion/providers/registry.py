...
# Mercadona DIRECT public-API crawl (free; replaces the paid Apify actor). Provider code stays
# ``apify-mercadona`` so LIVE mappings/candidates/prices are preserved. Only usable when
# ``mercadona_connector_enabled`` + a postal code are set; otherwise it raises on iterate.
register_default(MercadonaProvider.provider_code, MercadonaProvider)
