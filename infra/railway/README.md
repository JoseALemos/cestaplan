...
| `open-prices-sync` | [`open-prices-sync.json`](open-prices-sync.json) | `/` (raíz) | `apps/api/Dockerfile` | **No** | **Cron diario** (`0 4 * * *` UTC). Reutiliza la imagen del `api`; `startCommand` = `python -m cestaplan_api.scripts.sync_all_sources` (sincroniza precios reales ODbL de Open Prices y enriquece con Open Food Facts; `--no-enrich` omite el enriquecimiento). Sin dominio, sin healthcheck. |
...
| `db-backup`      | [`db-backup.json`](db-backup.json) | `/` (raíz) | `apps/api/Dockerfile` | **No** | **Cron diario** (`0 2 * * *`). `python -m cestaplan_api.jobs.backup_database` (pg_dump a `BACKUP_DIR`). ⚠️ **REQUIERE un volumen persistente montado en `BACKUP_DIR` (por defecto `/backups`)** + vars `BACKUP_DIR`/`BACKUP_RETENTION_DAYS`; sin volumen los volcados caen en fs efímero y se pierden (el job avisa). Railway ofrece backups gestionados de Postgres como fuente primaria. `restartPolicyType: NEVER`. |
| `audit-purge`    | [`audit-purge.json`](audit-purge.json) | `/` (raíz) | `apps/api/Dockerfile` | **No** | **Cron semanal** (`0 3 * * 1`). `python -m cestaplan_api.jobs.purge_audit_logs --commit` (minimización RGPD: borra `audit_log` anteriores a `AUDIT_RETENTION_DAYS`, por defecto 365). Opt-in; el responsable confirma el periodo de retención según su política. `restartPolicyType: NEVER`. |
